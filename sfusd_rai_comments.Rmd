...
**EDIT**: A previous version assumed that "growth" as defined in the metric represented the change over time of the SBAC scores for a school. I later found out (from Paul Gardiner's [SFEDup blog](https://sfeducation.substack.com/i/137024258/growth-not-achievement)) that the state is developing a metric for the average growth in SBAC score for a cohort of students at each school, as described [here](https://www.cde.ca.gov/ta/ac/acctgrowthmod.asp). The state released a version of the metric for informational purposes only (to demonstrate the model) in 2021, and the first version that can be used for evaluation will be released in the Fall 2024 based on SBAC data from 2022 to 2024. 

```{r, include = FALSE}
sbac_grow <- read_excel("growthaggr.xlsx") %>% 
    filter(districtname == "San Francisco Unified", studentgroup == "ALL") %>% 
    mutate(schoolname = str_replace_all(schoolname, 
                                        c("San Francisco Public" = "SF Public",
                                          "Yick Wo" = "Wo (Yick)")),
           Measure_Short = recode(subject, ela = "SBAC ELA", math = "SBAC Math")) %>% 
    select(schoolname, Measure_Short, growthscore)

sch1 <- sort(unique(sbac_all$SCHNAME))
sch2 <- sort(unique(sbac_grow$schoolname))
sch_match <- data.frame(SCHNAME = sch1, 
                        schoolname = sch2[match(str_sub(sch1, 1, 5), str_sub(sch2, 1, 5))])

sbac_all <- inner_join(sbac_all, sch_match) %>% 
    inner_join(sbac_grow)
sbac_all <- mutate(sbac_all, mean = (Y1 + Y2 + Y3) / 3)
```
...
```{r, message = FALSE}
ggplot(sbac_all, aes(x = SES_p, y = growthscore, color = Measure_Short)) +
    labs(x = "Percentage of SED students", y = "Growth score (2021 state model)", color = "Assessment") +
    geom_point() +
    geom_smooth(method = "lm", alpha = 0.2) +
    scale_color_brewer(palette = "Dark2") +
    theme_cowplot() +
...
```{r, include = FALSE}
sbac_gr_mod <- lm(growthscore ~ (AA_p + AS_p + HI_p + SES_p) * Measure_Short, 
                  data = sbac_all)
summary(sbac_gr_mod)
sbac_mean_mod <- lm(mean ~ (AA_p + AS_p + HI_p + SES_p) * Measure_Short, 
                  data = sbac_all)
summary(sbac_mean_mod)
```
